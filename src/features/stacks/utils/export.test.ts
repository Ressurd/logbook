...
describe("스택 JSON 백업", () => {
  it("트래커와 이벤트, 시간대, ISO 시각을 모두 포함한다", () => {
    ...
    const backup = JSON.parse(createStackJsonBackup({
      trackers: [{ id: "t", name: "휴식", scheduleMode: "all_day", startMinute: 0, endMinute: 1440, totalCharges: 24, intervalDays: null, anchorDate: null, isActive: true, createdAt: at, updatedAt: at }],
