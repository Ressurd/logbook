...
`/stacks`는 하루 동안 여러 번 충전하거나, 4일·7일처럼 정한 날짜 간격마다 1회씩 누적하는 기능입니다. 예를 들어 04:00~24:00에 140회를 설정하면 약 8분 34초마다 1회가 충전됩니다. `N일마다 1회 누적`을 선택하면 첫 충전 날짜부터 지정한 일수마다 +1이 쌓이며 사용하지 않은 수량은 다음 주기로 이월됩니다. 모든 날짜 경계와 스케줄 계산은 `Asia/Seoul` 기준입니다.
...
주기형 충전일 = 첫 충전 날짜 + (충전 순번 - 1) × 주기 일수
주기형 현재 스택 = 첫 충전일부터 누적 충전된 횟수 - 전체 누적 사용 횟수
```
...
트래커 문서에는 이름, `all_day | custom_time | interval_days`, 시작/종료 분, 하루 충전 횟수(1~200), `intervalDays`, `anchorDate`, 활성 상태, 생성/수정 서버 시각을 저장합니다. 기존 트래커에는 새 필드가 없어도 읽을 수 있으며, 다음 수정 때 `null` 값으로 보완됩니다. 이벤트 문서에는 트래커 ID와 당시 이름, `charge | consume`, KST 날짜, 충전 인덱스, `1 | -1`, 실제 발생 시각과 생성 서버 시각을 저장합니다.
...
트래커의 시간 범위나 총 충전 횟수를 당일 중간에 수정해도 이미 생성된 불변 충전 이벤트의 예정 시각은 다시 쓰지 않습니다. 주기형의 첫 충전일·일수·시각 또는 운영 방식 자체를 바꾸면 기존 이력을 보존하기 위해 새 트래커를 만들고 기존 트래커를 비활성화하는 원자적 교체로 처리합니다.
